class Music {
  constructor() {
    this.apiURL = `https://search-itunes.now.sh/?term=${this.searchText}&media=music`;
    this.musicArr;
    this.init();
  }
  ...
  async getData() {
    ...
    this.musicArr = resJSON.results;
    this.renderTracks(this.musicArr);
    this.sorting();
  }
  ...
  renderTracks(tracks = []) {
   ...
   <div class='row no-gutters'>
        <div class='col'><img src='${x.artworkUrl100}'></div>
            ...
            <div class='col'>
               <small class='text-muted'>ARTIST</small> <a href=${
                 x.trackViewUrl
               } target='_blank'><i class="fab fa-apple"></i></a><br>
               ${x.artistName} 
            </div>
    ...
    <div class='row no-gutters h4'><p class='card-text'>${
      x.trackName
    } </p></div>
    </div>
   ...
   <div class='info'>
      ...
      showMore.forEach(x =>
        x.addEventListener("click", () => {
          x.parentElement.parentElement.nextElementSibling.classList.toggle(
            "toggle"
          );
        })
      );
  ...
  sorting() {
    ...
    price.addEventListener("click", () => {
      let sorted = this.musicArr.sort((a, b) => a.trackPrice - b.trackPrice);
    ...
    length.addEventListener("click", () => {
      let sorted = this.musicArr.sort(
        (a, b) => a.trackTimeMillis - b.trackTimeMillis
      );
    ...
    release.addEventListener("click", () => {
      let sorted = this.musicArr.sort(
        (a, b) =>
          new Date(a.releaseDate.substr(0, 10)) -
          new Date(b.releaseDate.substr(0, 10))
      );
    ...
    document.querySelector("input").addEventListener("keyup", e => {
      ...
      let filteredMusic = this.musicArr.filter(
        track =>
          track.artistName.toLocaleLowerCase().includes(keyword) ||
      ...
      this.renderTracks(filteredMusic);
    });
  }
}
